...
if __name__ == '__main__':
    for fps in ['30']:
        for smpl_fn in [
            ...
            in_file = '/Users/renjiawei/Downloads/aist/motions/{}'.format(smpl_fn)
            traj_state_file = '../expert_aist/{}.npy'.format(smpl_fn)
            with open(in_file, 'rb') as f:
                data = pickle.load(f)
            print(data.keys())
            n_frames = 400
            ase_poses = data['smpl_poses'].reshape(-1, 24, 3)[:n_frames]  # (N, 24, 3)
            pelvis_trans = data['smpl_trans'].reshape(-1, 3)[:n_frames]  # (N, 24, 3)
            ...
            # trans_scale = abs_trans[0,2] / (pelvis_trans[0, 2] - floor_height)
            # print(floor_height, trans_scale, 1. / data['smpl_scaling'][0])
